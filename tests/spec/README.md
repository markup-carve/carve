...
This is a **derived directory** — do not edit the files here. Edits to the sources under [`resources/examples/`](../../resources/examples/) (`core.md`, `extensions.md`, `edge-cases.md`) regenerate both `../corpus/*.{crv,html}` and `*.test` on the next `npm run corpus:build`. Edits made directly to `.test` files will be overwritten.
