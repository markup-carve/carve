...
test('refusing at decode is the conformant answer', () => {
  const shape = refusableRootShapes(tree())[0]
  assert.equal(rootShapeVerdict({ shape, refused: true, renderRefused: false }), null)
})
...
test('refusing only in the renderer is a finding, and says so', () => {
  ...
  const verdict = rootShapeVerdict({ shape, refused: false, renderRefused: true })
