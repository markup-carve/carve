...
| `::: note {.x}` (trailing attribute on a typed colon-fence opener) | The trailing `[attributes]` block applies to the div/aside: classes accumulate after the type class (`class="admonition note x"`), `#id` / `key=value` set on the element; the block may abut the type or quoted title. A preceding block-attribute line merges (leading classes first, the opener winning on id/key conflict). Grammar `admonition_open` carries the `[attributes]` slot. carve-php applied it; carve-js gained it (markup-carve/carve-js#135). *(13-admonitions)* |
