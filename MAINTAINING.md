...
| `> quoted`<br>`continued` — lazy blockquote continuation | A non-`>` nonblank line continues the quote only while its trailing paragraph is open. Definitions, comments, attributes, and other block-looking bytes remain paragraph text there; a blank or enclosing structural boundary ends the paragraph. Matches Djot upstream. *(77-blockquote-lazy-continuation)* |
