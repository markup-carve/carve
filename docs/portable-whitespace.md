...
This costs nothing. The portable form below is also the CommonMark-safe
form, so following this page does not trade Markdown compatibility for Djot
...
line as paragraph text. A quote marker indented that way is already rendering
wrongly in Carve, so it is a plain correctness problem rather than a
portability one.
