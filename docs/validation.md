...
| `table-alignment-run-padding` | a table alignment marker run with no terminating space, such as `|>text`; the run is literal under the current grammar. `fmt --migrate` inserts the required space |
