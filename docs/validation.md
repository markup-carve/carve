...
| `semantic-attribute-value-ignored` | a non-empty value on `samp`, `var`, `kbd` or `cite` in a compact semantic span; those four names select their wrapper and the value reaches no output at all, so `[Dune]{cite="https://example.org/dune"}` renders `<cite>Dune</cite>` and loses the URL (PART 9 §10) |
| `semantic-attribute-outside-span` | one of the seven semantic names used where PART 9 §10 does not apply - a code span, link, image, block-attribute line or table cell - where it stays an ordinary attribute, so `` `c`{kbd} `` is `<code kbd="">` rather than a `<kbd>` |
