...
six single-character delimiters. So a doubled delimiter is always literal:
`**x**`, `~~x~~`, `^^x^^`, and `==x==` render verbatim, exactly like `//x//`
and `__x__` (corpus `71-doubled-emphasis-delimiters`). `==x==` literal is the
Design-A migration trigger: highlight is now single-char `=`, so the old
`==highlight==` must be rewritten `==` → `=`.
