...
- `boolean` - a bare word (no `#`/`.`/`=`) is a value-less attribute. In the AST it has an empty-string value, so `{open}` and `{open=""}` are equivalent and both render `open=""`. `carve fmt` writes that value back in the bare form, so `{open=""}` becomes `{open}`. The bare name may not start with `_`: `{_x_}` is a forced underline, so a bare `{_foo}` is text and an underscore attribute must keep its value (`{_k=""}`). An id, a class or a key/value may start with `_` as before. Three reserved names on inline spans instead select their HTML element (e.g. `[Tab]{kbd}` → `<kbd>Tab</kbd>`).
...
- **Only HTML changes.** The AST keeps an ordinary span carrying the authored attributes, plain-text and terminal output render the content, and `carve fmt` writes the span back out with its attributes - an empty-string one bare, so `[Tab]{kbd}` formats to itself. An empty-string attribute whose name starts with `_` keeps its `=""`, since the bare form would be an underline.
