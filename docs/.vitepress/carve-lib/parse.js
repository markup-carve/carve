...
class Lexer {
    ...
    commentNoCloserOfLenFrom = new Map();
...
function parseBlockSource(source, opts, root) {
    ...
    const sourceLineMap = sourceLines.length > 0 &&
...
function commentBlockHasCloser(lexer, fence) {
    ...
    if (start >= (lexer.commentNoCloserOfLenFrom.get(fence) ?? Infinity))
        return false;
    ...
    if (start < prev)
        lexer.commentNoCloserOfLenFrom.set(fence, start);
    return false;
}
