...
function parseParagraph(lexer) {
    ...
    while (!lexer.eof()) {
        ...
        const isInvisible = RE_LINK_DEF.test(ln) ||
        ...
        if (divOk && isBlockStart(ln) && (lexer.nested || isInvisible))
            break;
        lexer.consume();
        lines.push(ln);
    }
        ...
        if (c === '[') {
            ...
            if (close > 0) {
                ...
                if (mref && innerText !== '') {
                    ...
                    if (mref[2])
                        refLink.attrs = parseAttrs(mref[2]);
                    out.push(refLink);
            ...
            if (close > 0) {
                ...
                if (ms) {
                    ...
                    if (!isEmptyAttrs(attrs)) {
                        flush();
                        out.push({ type: 'span', children: scanInline(innerText), attrs });
